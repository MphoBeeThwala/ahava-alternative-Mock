...
const authKeyStrategy = (process.env.AUTH_RATE_LIMIT_KEY_STRATEGY || 'ip').toLowerCase();
...
const getAuthRateLimitKey = (req: any): string => {
  ...
  if (authKeyStrategy === 'ip_or_email' && emailHash) return `ip:${ip}|email:${emailHash}`;
  return `ip:${ip}`;
};
...
const generalMax = envInt(
  'RATE_LIMIT_MAX',
  process.env.LOAD_TEST === '1' ? 50000 : process.env.NODE_ENV === 'production' ? 100 : 10000
);
