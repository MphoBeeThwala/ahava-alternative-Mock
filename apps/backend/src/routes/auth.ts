...
router.post("/register", authRateLimiter, async (req, res, next) => {
  try {
    ...
    // Security: Restrict ADMIN, DOCTOR, and NURSE registration to holders of the correct secret key.
    // PATIENT accounts are always free and open.
    if (role === "ADMIN") {
      if (
        !adminSecret ||
        ...
        return res
          .status(403)
          .json({
            error: "Admin registration requires a valid registration key.",
          });
      }
    }
    if (role === "DOCTOR" || role === "NURSE") {
      if (
        ...
        return res
          .status(403)
          .json({
            error: "Staff registration requires a valid registration key.",
          });
      }
    }
