...
2026-09-24. Row 11 — same clinician, HPCSA MP1325247, 2026-09-25 (separate
session, one day later, since row 11 didn't exist yet on the 24th). Every
gated row on this checklist now has a signature. Signing is the clinical
attestation; it does not by itself flip the corresponding env var in any
running environment — that's a separate deployment step. As of this
record, `WHO_2019_CHART_SIGNED_OFF` and `BP_CHECK_PROMPT_SIGNED_OFF` are
set to `true` on the ML service in Railway production (verified directly,
`ENGINEERING_PLAN.md` §31); `FRAMINGHAM_LAB_CHART_SIGNED_OFF` is not yet
set anywhere.
