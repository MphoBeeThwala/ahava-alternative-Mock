...
| 7 | WHO 2019 chart — actual cell values | same function | The chart itself is a colour-coded image in the primary source, not yet transcribed to data | **Structurally blocked regardless of sign-off** — `computable: false`, `reasons_not_computable: ["WHO_2019_CHART_NOT_YET_DIGITIZED"]`. Primary source now sourced and saved (`docs/references/`, high-res crops in `docs/references/chart-crops/`) — a manual transcription attempt was made and abandoned after catching self-inconsistent results across repeat reads of the same image (`ENGINEERING_PLAN.md` §27). Transcription itself, done reliably, is the remaining blocker — not sign-off. |
