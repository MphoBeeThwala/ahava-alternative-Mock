...
## 34. FRAMINGHAM_LAB_CHART_SIGNED_OFF verified live; real integration testing against a real (disposable) Postgres, and a real bug it found

**Railway verified directly**, same method as §31: `railway variable list --kv` on the ML service shows `BP_CHECK_PROMPT_SIGNED_OFF=true`, `WHO_2019_CHART_SIGNED_OFF=true`, and `FRAMINGHAM_LAB_CHART_SIGNED_OFF=true`, with a fresh `SUCCESS` deployment (2026-09-25T12:13:28Z) running all instances. All three gates this codebase now has are live in production.

**Chose a real, disposable database over the real production one.** Asked to test "against live postgres," and this repo already has exactly the right tool for that — `pnpm test:integration` (`jest.integration.config.js`) spins up a genuine `pg_ctl`-managed Postgres via `embedded-postgres` when `DATABASE_URL` isn't set, and runs the real migration history against it with `prisma migrate deploy` (the same command CI's own migration-drift check uses). That's a real, live Postgres process, not a mock — just not the production one. Running an untested test suite directly against real patient data, on a request that could reasonably mean either, isn't a default I'll make silently; this gets the same verification value without the risk, and the option to actually target production remains open if that's specifically wanted.

**First blocker: `@node-rs/bcrypt` wasn't installed at all**, not just missing types — this has been a known, disclosed gap since before this session (git history: "Fix Railway backend build: regenerate package-lock.json for the @node-rs/bcrypt swap"), and it was blocking 10 of 11 integration suites from even compiling. Ran `pnpm install` in `apps/backend`; it installed cleanly.

**That fix surfaced a second, previously-hidden real error**: with bcrypt no longer failing first, `tsc --noEmit` found `doctorMonitoring.ts`'s `const router = Router()` producing an unnamed/unportable inferred type (TS2742) — every earlier `tsc` run this session had passed clean on this exact file, because the bcrypt resolution failure was short-circuiting TypeScript before it got far enough to report this. Fixed with the same explicit `const router: Router = Router()` annotation `bpValidation.ts` and `visits.ts` already used. A reminder that a clean `tsc` run is only as complete as what it actually got to check.

**Baseline confirmed**: the full pre-existing integration suite (11 files, 112 tests) ran clean against the real disposable Postgres — meaningfully, this is the first time this session's three new Prisma migrations (`20260923120000_add_visit_id_to_biometric_readings`, `20260924120000_add_bp_risk_retrospective_fields`, `20260924150000_add_framingham_lab_fields`) were actually applied via `prisma migrate deploy` against a real database, not just `prisma validate`/`generate` against a schema with no database behind it.

**New coverage written for everything §29/§30/§32 built with none**:
- `patient.integration.test.ts` — `GET /early-warning`'s fallback response shape, pinned field-by-field against the real ml-service contract (`instrument`, `computable`, `risk_category`, `ten_year_risk_pct`, `risk_bound`, the `bp_risk` disclaimer text) — the exact shape §29 rewrote after finding the frontend reading fields real traffic never sent. A regression here would silently reintroduce that bug. Also confirms the retrospective-snapshot fields persist as `null`/`false` correctly in the fallback case.
- `visits.integration.test.ts` — the nurse calibration endpoint (`POST /:id/biometrics`): success on an in-progress visit persisting `visitId`/`deviceType: "nurse_calibration"`, and refusal on a non-in-progress visit, an unassigned nurse, an out-of-range value, and a patient caller.
- `doctorMonitoring.integration.test.ts` (new file) — role/auth gates, consent-gating (a flagged patient without `BIOMETRIC_MONITORING` consent never appears), and specifically the "most recent reading, not any historical match" behavior #30 was careful to get right: a patient whose latest reading is GREEN doesn't surface just because an older one was RED.
- `bpValidation.integration.test.ts` (new file) — role gates (admin-only, a doctor is correctly denied), and the pairing/classification logic itself.

**The pairing tests found a real bug, not a test artifact.** `bpValidation.ts`'s candidate-reading query had `NOT: { deviceType: 'nurse_calibration' }` alongside `bpPromptCheck: { not: null }`. In SQL, `deviceType <> 'nurse_calibration'` evaluates to `NULL` — not `TRUE` — for any row where `deviceType IS NULL`, so a bare `WHERE` clause silently drops those rows instead of keeping them. Since `deviceType` is unset on nearly every real biometric reading (only nurse-calibration and a handful of wearable-sourced rows populate it at all), this meant the validation report §32 built was, as shipped, silently excluding almost all real candidate data — a real production correctness bug the report's own author (this session) would not have caught without an actual database enforcing actual SQL semantics. Fixed by removing the clause outright rather than patching its NULL-handling: `bpPromptCheck: { not: null }` was already sufficient on its own, since a calibration reading never populates `bpPromptCheck` in the first place. First two test runs caught this precisely (`pair` was `undefined` where a same-user, same-window pair should have existed); third run, after the fix, passed.
